...
static char g_input_dir[MAX_PATH] = {0};  // Remember the input directory for relative paths
...
void add_file(const char* path) {
	...
	if (g_files.count >= g_files.capacity) {
		...
		g_files.files = (SourceFile*)realloc(g_files.files, 
			g_files.capacity * sizeof(SourceFile));
	}
...
void extract_includes(SourceFile* file, char includes[][MAX_PATH], int* include_count) {
	*include_count = 0;
...
void process_file(SourceFile* file, const char* chain[], int chain_len) {
	...
	g_order.paths = (char**)realloc(g_order.paths, (g_order.count + 1) * sizeof(char*));
...
int main(int argc, char** argv) {
	...
	*ptr = '\0';
	...
	// Build file index: for each file, track its line range in the bundled output
	typedef struct {
		SourceFile* file;
		char rel_path[MAX_PATH];
		int start_line;
	...
	void get_line_at(const char* content, int line_num, char* output, size_t max_len) {
		int current = 1;
	...
	// Find the line in source file by searching for matching lines with expanding context
	// Uses alternating up/down search: try error line, then error-1, error+1, error-2, error+2, etc.
	int find_line_in_source(SourceFile* file, const char* bundled_content, int error_bundled_line, FileIndex* file_index) {
		...
		int file_start_offset = file_index->start_line - 1;  // Convert to 0-based offset from bundled start
		
		// Extract context lines by alternating up and down from error
		int bundled_offset = error_bundled_line - 1;  // 0-based
		
		for (int radius = 0; radius < max_context && context_line_count < max_context * 2; radius++) {
			...
			if (radius > 0) {
				...
				if (above_line >= file_index->start_line) {
					...
					if (strlen(line) > 0) {
						...
						context_line_offsets[context_line_count] = above_line - file_index->start_line + 1;  // Relative to file start
						context_line_count++;
					}
				}
			}
			...
			if (below_line < file_index->end_line) {
				...
				if (strlen(line) > 0) {
					context_lines[context_line_count] = strdup(line);
					context_line_offsets[context_line_count] = below_line - file_index->start_line + 1;
					context_line_count++;
				}
			}
			...
			for (int i = 0; i < context_line_count; i++) {
				...
				if (occurrences == 1) {
					...
					for (int j = 0; j < context_line_count; j++) {
						if (context_line_offsets[j] == bundled_offset - file_index->start_line + 1) {
	...
	if (as_build(validator, errors) < 0) {
		...
		for (int i = 0; i < errors->error_count; i++) {
			...
			if (found_file && file_idx >= 0) {
				...
				original_line = find_line_in_source(found_file, bundled, errors->errors[i].line, &index[file_idx]);
			}
